...
import { fetchProductPlanData, selectProductPlanData, selectProductPlanLoading, selectProductPlanError, addProductPlan, editProductPlan, deleteProductPlan } from '../../../../features/slices/productPlanSlice';
import { fetchCategoryData, selectCategoryData } from '../../../../features/slices/categorySlice';
...
const ProductsPlan = () => {
  ...
  const isLoading = useSelector(selectProductPlanLoading);
  const error = useSelector(selectProductPlanError);
  const role = useSelector(selectSelectedRole);
  const roleId = localStorage.getItem('role');
  const products = useSelector(selectCategoryData);
  // State management
  ...
  const handleSubmit = (e) => {
    ...
    if (isAddModalOpen) {
      dispatch(addProductPlan(formData)).then(() => {
        setIsSubmitting(false);
        closeAllModals();
        dispatch(fetchProductPlanData());
      });
    } else if (isEditModalOpen && selectedPlan) {
      dispatch(editProductPlan({ id: selectedPlan.id, ...formData })).then(() => {
        setIsSubmitting(false);
        closeAllModals();
        dispatch(fetchProductPlanData());
      });
  ...
  const handleDelete = () => {
    setIsSubmitting(true);
    dispatch(deleteProductPlan(selectedPlan.id)).then(() => {
      setIsSubmitting(false);
      closeAllModals();
      dispatch(fetchProductPlanData());
    });
  ...
  const filteredPlans = (productPlanData || []).filter(plan =>
  ...
  const productPlanPerm = getModulePermission(permissions, 'product plans');

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Product Plans Management</h1>
        <p className="text-gray-500">Manage your home product plans</p>
      </div>
                ...
                <div key={plan.id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
                  ...
                  <div className="p-4 flex-grow flex flex-col">
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-semibold text-gray-800">{products.find(p => p.id === plan.productId)?.name || '-'}</h3>
                      {plan.highlight && (
                      ...
                      <tr key={plan.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="ml-0">
                              <div className="text-sm font-medium text-gray-900">{products.find(p => p.id === plan.productId)?.name || '-'}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 md:mx-auto max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b">
              <h3 className="text-lg font-medium text-gray-900">Add Product Plan</h3>
              <button 
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <button type="button" onClick={addFeatureField} className="text-blue-600 hover:text-blue-800 text-sm font-medium flex items-center">
              ...
              <div className="px-6 py-4 bg-gray-50 text-right rounded-b-lg">
                <button type="button" className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mr-2" onClick={closeAllModals}>Cancel</button>
                <button type="submit" className="bg-blue-600 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" disabled={isSubmitting}>{isAddModalOpen ? 'Save Plan' : 'Update Plan'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 md:mx-auto max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b">
              <h3 className="text-lg font-medium text-gray-900">Edit Product Plan</h3>
              <button 
                className="absolute top-4 right-4 text-gray-400 hover:text-gray-500"
                onClick={closeAllModals}
              >
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-4 md:mx-auto">
            <div className="px-6 py-4 border-b">
              <h3 className="text-lg font-medium text-gray-900">Delete Product Plan</h3>
              <button 
                className="absolute top-4 right-4 text-gray-400 hover:text-gray-500"
                onClick={closeAllModals}
              >
            ...
            <div className="p-6">
              ...
              <p className="text-gray-700 text-center mb-4">
                Are you sure you want to delete the <span className="font-semibold">{selectedPlan?.title}</span> product plan? This action cannot be undone.
              </p>
            </div>
            <div className="px-6 py-4 bg-gray-50 text-right rounded-b-lg">
              <button
